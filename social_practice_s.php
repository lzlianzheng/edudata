...
<script type="text/javascript">
	function scholar_in(id)
	{
		...
		{
			$.get("./scholar_insert.php?act=social_practice&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data)
			{
				alert(data);
			});
		}
...
<div class="menu tc">
　
<a href="social_practice.php">返回</a>　
<form action="social_practice_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "
					...
					echo "</select></td>
					<td><select name='per_jibie'><option value=''>请选择</option>";
					foreach($per_jibie as $s)
					{
						echo "<option value='$s'>$s</option>";
					}
					echo "</select></td>
