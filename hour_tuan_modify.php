...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from hour_tuan a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
<div class="menu tc">
<br><h2 style="text-align:center;display:inline-block">优秀团员团干修改</h2><br>
<a href="<?php echo "hour_tuan.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>　
</div>
<form action="modify.php?act=hour_tuan" method="post">
...
<tr>
	...
	<td align=center ><select name="xuenian">
	<?php
...
<tr>
	<td align=center>优秀团员</td><td align=center><select name='tuanyuan'>
	<option <?php if("$result->tuanyuan" == 1){echo "selected";} ?> value="1">是</option>
	<option <?php if("$result->tuanyuan" != 1){echo "selected";} ?> value="0">否</option></td>
	</select>
	<td align=center>优秀团干</td><td align=center><select name='tuangan'>
	<option <?php if("$result->tuangan" == 1){echo "selected";} ?> value="1">是</option>
	<option <?php if("$result->tuangan" != 1){echo "selected";} ?> value="0">否</option></td>
</tr>
<tr>
