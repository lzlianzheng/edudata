...
$level=$yiqi_db->get_results('select grade from honour_type where id in(14,15,16,17,18)');
$title="单项奖学金录入";
include("header.php");
?>
...
<form action="scholar_single_insert.php" method="get" class="disin">
...
if(count($result) > 0){
	foreach($result as $v){
		echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
			...
			//未对数据库进行操作
			echo "<td>	
			<textarea name='beizhu'>$v->beizhu</textarea></td>";
			echo "<td>	
