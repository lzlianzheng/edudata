...
if($_REQUEST['act']=='stat')
{
	...
	$sql2="select c.*,d.id from scholarship c join ($sql) d on c.sid=d.sid ";

//$level=$yiqi_db->get_results('select * from scholarship' );
...
<form action="scholar_result_update.php" method="get" class="disin">
...
<form action="scholar_result_update.php" method="post">
...
if(count($result)> 0){
	foreach($level as $v){
		echo "<tr>
			...
			echo "<input type='hidden' name='class' value='$v->scholarship'>$v->scholarship</td>";
		
			echo "<td>	
			<a href='scholar_result_update_d.php?sid=$v->sid'>修改</a>　
			<a href='scholar_del.php?act=scholar2_delect&sid=$v->sid'>删除</a></td>";
				
			
			echo "<td><input type='checkbox'  name='chk[]' value='$v->sid'></td>";
	}
