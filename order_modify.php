...
$id = $_GET['id'];
print_r($id);
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from order_info a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
<div class="menu tc">
<br><h2 style="text-align:center;display:inline-block">专业排名修改</h2><br>
<a href="<?php echo "order.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>
</div>
<form action="modify.php?act=order" method="post">
...
<tr>
	<td align=center>排名</td><td align=center >
		<select name="mingcheng">
	<?php
	foreach($jibie as $v){
