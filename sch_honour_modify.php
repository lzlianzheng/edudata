...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from honour a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
<div class="menu tc">
<br><h2 style="text-align:center;display:inline-block">校级荣誉信息修改</h2><br>
<a href="<?php echo "sch_honour.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>　
</div>
<form action="modify.php?act=sch_honour" method="post">
...
<tr>
	<td class="w10" align=center>级别</td><td colspan=4 align=center>校级</td>
</tr>
