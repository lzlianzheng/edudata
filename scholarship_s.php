...
function scholar_in(id){
	...
	if(!year || !xueqi){
		alert("请输入奖学金记录所属学年/学期");
	}else{
		$.get("./scholar_insert.php?act=scholarship&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
	...
	<form action="scholarship_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					<td><select name='scholarship'><option value=''>请选择</option>";
					foreach($jibie as $s)
					{
