...
function scholar_in(id){
	...
	if(!year || !xueqi){
		alert("请输入晚自习个人记录所属学年/学期");
	}else{
		$.get("./scholar_insert.php?act=wanzixi_personal&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
　
<a href="wanzixi_personal_record.php">返回</a>　
<form action="wanzixi_personal_s.php" method="get" class="disin">
...
if(count($result) > 0){
	foreach($result as $v){
		echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
			...
			<td><input type='text' name='kuangkeriqi' value='' size=15/></td>
			<td><input type='text' name='chidaoriqi' value='' size=15></td>
			<td><input type='text' name='zaotuiriqi' value='' size=15></td>
