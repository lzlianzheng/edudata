...
$level=$yiqi_db->get_results('select grade from honour_type where type="奖学金等级"');
$sch_honour=$yiqi_db->get_results('select grade from honour_type where type="校级荣誉"');
$yuanji_honour=$yiqi_db->get_results('select grade from honour_type where type="院级荣誉"');
$yuanji_scholar=$yiqi_db->get_results('select grade from honour_type where type="院奖学金"');
...
<form action="scholar.php" method="get" class="disin">
...
if(count($result) > 0){
	foreach($result as $v){
		echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
			...
			echo "</td>";
			echo "<td>";
			foreach($yuanji_honour as $k=>$v){
			echo "$v->grade<input type='checkbox' name='yuanji[]' value='$v->grade'>	";
			}
			echo "</td>";
			echo "<td>";
			foreach($yuanji_scholar as $k=>$v){
			echo "$v->grade<input type='checkbox' name='yjscholar[]' value='$v->grade'>	";
			}
			echo "</td>";
			echo "<td>	
