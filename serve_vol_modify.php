...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from serve_vol a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
?>
<div class="menu tc">
<h2 style="text-align:center;display:inline-block">志愿服务修改</h2>
<br><a href="<?php echo "serve_vol.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>
</div>
<form action="modify.php?act=serve_vol" method="post">
...
<tr><td class="w10" align=center>级别</td>
	<td align=center ><select name="jibie">
	<?php
	foreach($jibie as $v){
...
<tr>
	<td align=center >服务日期</td>
	<td align=center ><input type="text" name="serdate" value="<?php echo $result->serdate; ?>" /></td>
	<td align=center >服务时长</td>
	<td align=center ><input type="text" name="sertime" value="<?php echo $result->sertime; ?>" /></td>
</tr>
<tr>
