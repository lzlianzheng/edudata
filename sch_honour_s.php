...
function scholar_in(id){
	...
	if(!year || !xueqi){
		...
		$.get("./scholar_insert.php?act=sch_honour&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
	<a href="sch_honour.php">返回</a>　
	<form action="sch_honour_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					<td><input type='hidden' name='jibie' value='校级' size=5 />校级</td>
					<td>三好学生　<input type='checkbox' name='sanhao' value='1' size=15 /></td>
					<td>优秀学生干部　<input type='checkbox' name='yougan' value='1' size=15 /></td>
