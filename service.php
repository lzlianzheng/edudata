...
function service_in(id){
	year = $("input[name=year]").val();
	xueqi = $("select[name=xueqi]").val();
	if(!year || !xueqi){
		alert("请输入奖励所属学年/学期");
	}else{
		$.get("./service_insert.php?act=save&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
　
<a href="vol_service.php">返回</a>　
<form action="service.php" method="get" class="disin">
...
if(count($result) > 0){
	foreach($result as $v){
		echo "<form id='vl$v->sid' method='get' onsubmit='service_in(\"vl$v->sid\");return false;'><tr>
			...
			<td><input name='fuwutime' type='text' value='' maxlength='20'/></td>
			<td><select name='grade'><option value=''>请选择</option>";
