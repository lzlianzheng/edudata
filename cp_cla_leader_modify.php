...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.name as classname,d.id,d.name as majorname from cp_cla_leader a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid WHERE a.id = $id ";
...
?>
<div class="menu tc">
<br><h2 style="text-align:center;display:inline-block">班干部测评结果修改</h2><br>
<a href="<?php echo "cp_cla_leader.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>　
</div>
<form action="modify.php?act=cp_cla_leader" method="post">
...
<?php
	echo "<tr><td align=center>测评等级：</td><td align=center><select name='cpdengji'>";
   foreach($cpdengji as $v)
    {
	  $se = ($v==$result->cpdengji)? " selected " : "";
	  echo "<option $se value='$v'>$v</option>";
    }
	echo "</select></td>";
?>
