...
function scholar_in(id){
	...
	if(!year || !xueqi){
		...
		$.get("./scholar_insert.php?act=hour_tuan&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
	<a href="hour_tuan.php">返回</a>　
	<form action="hour_tuan_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					echo "</select></td>";
					echo "<td>优秀团员　<input type='checkbox' name='tuanyuan' value='1' size=15 /></td>
					<td>优秀团干　<input type='checkbox' name='tuangan' value='1' size=15 /></td>
					<td><input type='submit' value='写入'/></td></tr></form>";
			}
		}		
		else{echo"<tr><td colspan='12'>暂无记录</td></tr>";}
	}	
	else{echo"<tr><td colspan='12'>请输入筛选个条件</td></tr>";}
?>
</table>
