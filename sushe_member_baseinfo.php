...
if($_GET["act"] == "display" || $_GET["act"] == "out")
{
	...
	$sql2 = "select a.classid,a.name,a.sid,a.mobilephone,b.name as classname from student_info a left join class b on a.classid = b.id where 1";
	if($dormlou)
...
if($_GET["act"] == "out")
{
	...
	$tnow = "<td>班级</td><td>学号</td><td>姓名</td><td>电话</td>";
	echo "<table border=1><tr>".$tnow."</tr>\n";
	if(count($data)>0){
		foreach($data as $k=>$v)
		{
		$outc .= "<tr><td>$v->classname</td><td>$v->sid</td><td>$v->name</td><td>$v->mobilephone</td></tr>";
		}
		echo $outc;
	}
	echo "</table>";
...
<a href="<?php echo "sushe_member_baseinfo.php?act=out&syear=$syear&dormlou=$dormlou&number=$dormnum"; ?>">导出数据</a><br/>	
<form action="sushe_member_baseinfo.php" method="get" class="disin">
...
if(count($data) > 0)
{
	foreach($data as $k=>$v)
	{
		echo "<tr><td>$v->classname</td><td>$v->sid</td><td>$v->name</td><td>$v->mobilephone</td></tr>";
