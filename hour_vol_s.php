...
<script type="text/javascript">
	function scholar_in(id)
	{
		...
		{
			$.get("./scholar_insert.php?act=hour_vol&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data)
			{
				alert(data);
			});
		}
...
<div class="menu tc">
	　
	<a href="hour_vol.php">返回</a>　
	<form action="hour_vol_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					echo "</select></td>
							<td><input type='hidden' name='mingcheng' value='1' size=5 />优秀志愿者</td>
