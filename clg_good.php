...
require_once 'getuid.php';	
	...
	$table = clg_good;
	$sql="select a.*,a.id as nid,a.year as xuenian,a.xueqi,b.name,b.sid,b.classid,c.year,c.id,c.majorid,c.name as classname,d.id,d.uid from clg_good a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on c.majorid = d.id where 1";
...
?>
	...
	<a href="clg_good_s.php">普通录入</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<a href="<?php echo "clg_good.php?act=out&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">导出数据</a>
	<br>
	<form action="clg_good.php" method="get" class="disin">
...
?>
	...
	<tr>
		<td><input id="slt" type="checkbox"/></td>
		<td>学号</td>
		<td>姓名</td>
		<td>班级</td>
		<td>级别</td>
		...
		if(count($result)>0)
		{
			foreach($result as $k=>$v)
			{
				echo"<tr>
					...
					<td>$v->xueqi</td>
					<td><a href='clg_good_modify.php?act=sel&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi&id=$v->nid'>修改</a></td>
					<td><a href='clg_good.php?act=delete&id=$v->nid' onClick='delete_confirm' >删除</a></td>";
				echo "</tr>";
			}
		}
