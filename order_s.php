...
function scholar_in(id){
	...
	if(!year || !xueqi){
		...
		$.get("./scholar_insert.php?act=order&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data){
			alert(data);
		});
	}
	return false;
...
<div class="menu tc">
	...
	<form action="order_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					<td><select name='mingcheng'><option>请选择</option>";
					foreach($jibie as $s)
					{
