...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from hour_su_leader a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
?>
<div class="menu tc">
<h2 style="text-align:center;display:inline-block">学生会干部任职修改</h2>
<br><a href="<?php echo "hour_su_leader.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>
</div>
<form action="modify.php?act=hour_su_leader" method="post">
...
<tr>
	...
	<td align=center ><input type="text" name="mingcheng" value="<?php echo $result->mingcheng; ?>" /></td>
</tr>
<tr>
