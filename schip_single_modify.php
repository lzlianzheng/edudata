...
{
$id = $_GET['id'];
$sql = "select a.id as nid,a.*,b.name,b.sid,b.classid,c.id,c.majorid,c.name as classname,d.id,d.name as majorname from schip_single a left join student_info b on a.sid = b.sid left join class c on b.classid = c.id left join major d on d.id = c.majorid where a.id = '$id' ";
...
<div class="menu tc">
<br><h2 style="text-align:center;display:inline-block">单项奖学金信息修改</h2><br>
<a href="<?php echo "schip_single.php?act=list&sname=$sname&sid=$sid&s_major=$major&s_year=$grade&s_class=$classn&xuenian=$sxuenian&xueqi=$sxueqi"; ?>">返回</a>　
</div>
<form action="modify.php?act=schip_single" method="post">
...
<tr>
	<td align=center >奖学金类型</td><td align=center ><select name="jibie">
	<?php
	foreach($jibie as $v){
	...
	?></select></td>
	<td align=center>获奖原因</td><td align=center><textarea name='cause'><?php echo $result->cause; ?></textarea></td>
</tr>
<tr>
