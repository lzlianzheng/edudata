...
<script type="text/javascript">
	function scholar_in(id)
	{
		...
		{
			$.get("./scholar_insert.php?act=cp_su_ganshi&year="+year+"&xueqi="+xueqi,$("#"+id).serialize(),function(data)
			{
				alert(data);
			});
		}
...
<div class="menu tc">
	　
	<a href="cp_su_ganshi.php">返回</a>　
	<form action="cp_su_ganshi_s.php" method="get" class="disin">
		...
		if(count($result) > 0)
		{
			foreach($result as $v)
			{
				echo "<form  id='sl$v->sid' method='get' onsubmit='scholar_in(\"sl$v->sid\");return false;'><tr>
					...
					echo "</select></td>
							<td><input type='hidden' name='mingcheng' value='1' size=5 />优秀学生会干事</td>
